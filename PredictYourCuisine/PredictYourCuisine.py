...
for i in train_ingredients:
    word = i
    for w in word:
        if w not in sum_ingredients:
           sum_ingredients[w] = 1
        else:
           sum_ingredients[w] = sum_ingredients[w] + 1
print(Counter(sum_ingredients).most_common(10))
...
italian_ingredients = {}
italian_ingredients_array = (train_content[train_content['cuisine'] == 'italian'])['ingredients']
for x in italian_ingredients_array:
    for find in x:
        if find not in italian_ingredients:
            italian_ingredients[find] = 1
        else:
            italian_ingredients[find] = italian_ingredients[find] + 1
print(Counter(italian_ingredients))
...
# classifier.fit(train_tfidf, train_targets)
